...
def read_config ():
	...
	for node in config_dom.getElementsByTagName("entry"):
		...
		if (node.getAttribute("active") != "0"):
			active[node.getAttribute("dirname")] = node.getAttribute("url")
...
def update_data ():
	...
	for table in tables:
		for dirname in entries.keys():
			if dirname not in active.keys() or dirname in update.keys():
	...
	for dirname in update.keys():
		...
		for file in os.listdir(path):
			if (file[:-4] not in tables):
				print "Warning: Unrecognized data file " + file + " found. Skipping it."
			else:
...
def update_data_file (path, file):
	...
	for row in csv_reader:
		...
		#print values
		if (row_count > 0):
			queryPred.append(insert_row (table, columns, dict(zip(columns, values))))
		row_count += 1

	if (is_data):
		i = 0
		while (i < len(queryPred)):
			query = "insert into " + table + " ("
			for name in columns:
				query += name + ", "
			query += "agency_str) values "
			for j in range(1000):
				if (i < len(queryPred)):
					query += queryPred[i]
				i += 1
...
def insert_row (table, columns, values):
	...
	if (table in unique_fields.keys() and unique_fields[table] in values.keys()):
